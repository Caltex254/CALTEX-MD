...
<body>
  ...
  // ---------------------------------------------------------------------------
  // v1.7.0: Smarter pairing flow with rate-limit detection.
  //   1. Call createConnectionWithPairingCode
  //   2. If a pairing code is returned, print the clean banner and return.
  //   3. If pairing fails with isRateLimited=true (WhatsApp 401 loggedOut /
  //      device_removed), DON'T retry — print a clear "rate-limited" message
  //      and exit cleanly (process.exit 0) so the panel stops auto-restarting.
  //      The user must wait 1-12 hours for WhatsApp's rate-limit to expire,
  //      then manually click "Start" on the panel.
  //   4. If pairing fails for other reasons (network blip, etc.), retry with
  //      a 60-second delay (was 10s — too aggressive, made rate-limit worse).
  //   5. After MAX_PAIRING_RETRIES (3) failed attempts, print a clean error
  //      and exit cleanly.
  private async connectWithPairingCode(sessionId: string, phoneNumber: string): Promise<void> {
    const MAX_PAIRING_RETRIES = 3;
    const RETRY_DELAY_MS = 60_000; // v1.7.0: was 10s, now 60s to avoid WhatsApp rate-limit
    ...
    for (let attempt = 1; attempt <= MAX_PAIRING_RETRIES; attempt++) {
      try {
        ...
        const { sock, pairingCode, pairingFailed, failureReason, isRateLimited } = await this.connectionManager.createConnectionWithPairingCode(
          {
            sessionId,
            printQR: false,
            browser: 'CALTEX MD',
        ...
        if (pairingFailed) {
          // v1.7.0: If WhatsApp rate-limited (401 loggedOut / device_removed),
          // DO NOT retry — exit cleanly so the panel stops auto-restarting.
          if (isRateLimited) {
            // eslint-disable-next-line no-console
            console.log('');
            // eslint-disable-next-line no-console
            ...
            console.log('');
            // eslint-disable-next-line no-console
            console.log('  Reason: rate-limited or device was removed.');
            // eslint-disable-next-line no-console
            console.log(`  Detail: ${failureReason}`);
            // eslint-disable-next-line no-console
            console.log('');
            // eslint-disable-next-line no-console
            console.log('  This happens when:');
            // eslint-disable-next-line no-console
            console.log('   • You previously removed this bot from your phone\'s');
            // eslint-disable-next-line no-console
            console.log('     WhatsApp → Settings → Linked Devices list.');
            // eslint-disable-next-line no-console
            console.log('   • Too many pairing attempts were made in a short time.');
            // eslint-disable-next-line no-console
            console.log('   • WhatsApp is temporarily blocking this server.');
            // eslint-disable-next-line no-console
            console.log('');
            // eslint-disable-next-line no-console
            console.log('  ⏳ Please WAIT 1 to 12 hours, then click "Start" on');
            // eslint-disable-next-line no-console
            console.log('     the panel to try again. Retrying now will only');
            // eslint-disable-next-line no-console
            console.log('     make the rate-limit longer.');
            // eslint-disable-next-line no-console
            console.log('');
            // eslint-disable-next-line no-console
            console.log('  Bot is shutting down cleanly (no auto-restart).');
            // eslint-disable-next-line no-console
            console.log('============================================================');
            // eslint-disable-next-line no-console
            console.log('');
            // v1.7.0: Exit code 0 tells pterodactyl-start.sh NOT to auto-restart.
            // The user must manually click "Start" on the panel after waiting.
            process.exit(0);
          }

          // Non-rate-limit failure — retry with longer delay
          if (attempt < MAX_PAIRING_RETRIES) {
            // eslint-disable-next-line no-console
            console.log(`Link failed (${failureReason}). Retrying in 60s... [attempt ${attempt}/${MAX_PAIRING_RETRIES}]`);
            await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
            continue;
          } else {
            // eslint-disable-next-line no-console
            console.log('Link failed. All retries exhausted. Please restart the server.');
            // v1.7.0: Exit cleanly so panel doesn't auto-restart in a tight loop.
            process.exit(0);
          }
        }

        if (pairingCode) {
          // Display the clean pairing code banner.
          ...
          if (attempt < MAX_PAIRING_RETRIES) {
            // eslint-disable-next-line no-console
            console.log(`Link failed (no code returned). Retrying in 60s... [attempt ${attempt}/${MAX_PAIRING_RETRIES}]`);
            await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
            continue;
          }
          // eslint-disable-next-line no-console
        ...
        if (attempt < MAX_PAIRING_RETRIES) {
          // eslint-disable-next-line no-console
          console.log(`Link failed (${err?.message || 'error'}). Retrying in 60s... [attempt ${attempt}/${MAX_PAIRING_RETRIES}]`);
          await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
          continue;
        }
        // eslint-disable-next-line no-console
